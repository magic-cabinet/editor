...
function schemaForNode(value: unknown) {
  ...
  return AnyNodeSchema
}

/** The `extra` bag `setScene` accepts — collections, materials, plugin state. */
type SetSceneExtra = Parameters<ReturnType<UseSceneStore['getState']>['setScene']>[2]
...
export class SceneBridge {
  ...
  setScene(
    ...
    this.sceneStore.getState().setScene(nodes, rootNodeIds, extra)
  }
  ...
  applyPatch(patches: Patch[]): {
    ...
    for (let i = 0; i < patches.length; i++) {
      ...
      if (p.op === 'create') {
        const res = schemaForNode(p.node).safeParse(p.node)
        if (!res.success) {
          throw new Error(
